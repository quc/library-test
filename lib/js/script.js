...
Library.prototype.init = function() {
  ...
  var books = this.getFromLocalStorage('books'),
      authors = this.getFromLocalStorage('author'),
      genres = this.getFromLocalStorage('genre'),
      settings = this.getFromLocalStorage('librarySettings');
...
Library.prototype.booksInit = function() {
...
function UI() {
  ...
  var userAgent = navigator.userAgent,
      pattern = new RegExp('mobile','gi'),
      isTouch = 'ontouchstart' in window && pattern.test(userAgent);

  this.startEvent = isTouch ? 'touchstart' : 'mousedown';
  this.moveEvent = isTouch ? 'touchmove' : 'mousemove';
  this.endEvent = isTouch ? 'touchend' : 'mouseup';
...
UI.prototype = {
  ...
  showBookPopup : function(el) {
    this.find('popup_wrapper').classList.add('open');

    this.find('popup').classList.add('open');
  },
  ...
  closeBookPopup : function(el, ev) {
    this.find('popup_wrapper').classList.remove('open');

    this.find('popup').classList.remove('open');
  ...
  setSorting : function () {
    ...
    if (sortReverse) {
      this.find('sort_reverse').classList.toggle('active');
    }

    this.booksFromStorage(books);
  },
  toggleSortList : function(el, ev) {
    this.find('sort_button').classList.toggle('active');

    this.find('sort_list').classList.toggle('open');
  },
  ...
  sortReverse : function(el, ev) {
    ...
    this.find(el).classList.toggle('active');
  },
  ...
  removeBook : function(el) {
    var cardBlock = el.parentNode.parentNode.parentNode;
